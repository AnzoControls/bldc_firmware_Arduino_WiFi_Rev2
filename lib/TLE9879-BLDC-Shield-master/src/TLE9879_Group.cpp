...
TLE9879_Group::TLE9879_Group(uint8_t boardCount)
{
	...
	// this->setParameter(FOC_MAX_SPEED,5000, BOARD1);
	...
	Serial.println("Slave select is set as output");
	...
	for(int i = 0; i < boardCount; i++)
	{
		...
		while(!boards[i]->isAvailable())
		{
			...
			delay(2000);
			if(status->code != ERR_NONE) {
				Serial.println("Status is not ERR_NONE");
				processStatusCodes(BOARDCONTROL, i + 1);
			}
